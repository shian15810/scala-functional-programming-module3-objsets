...
abstract class TweetSet extends TweetSetInterface:
   ...
   *
   * Hint: the method `remove` on TweetSet will be very useful. Question: Should
   * we implement this method here, or should it remain abstract and be
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def mostRetweetedAcc(acc: Tweet): Tweet =
    if elem.retweets > acc.retweets then
...
trait TweetList:
  ...
  def tail: TweetList
  def isEmpty: Boolean
  def foreach(f: Tweet => Unit): Unit = if !isEmpty then
    f(head)
    tail.foreach(f)
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets
    .filter(t => google.exists(e => t.text.contains(e)))

  lazy val appleTweets: TweetSet = TweetReader.allTweets
    .filter(t => apple.exists(e => t.text.contains(e)))

  /**
   * A list of all tweets mentioning a keyword from either apple or google,
   * sorted by the number of retweets.
   */
  lazy val trending: TweetList = googleTweets.union(appleTweets)
    .descendingByRetweet
end GoogleVsApple
